...
def main():
    ...
    out_df = pd.DataFrame()
    out_df['WF-URL'] = df['url']
    out_df['WF-prod-wd'] = df.apply(lambda row: combine_kvs(row, PROD_WD_COLS), axis=1)
    out_df['WF-prod-spec'] = df.apply(lambda row: combine_kvs(row, prod_spec_cols), axis=1)
